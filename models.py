...
class VisualEncoder(jit.ScriptModule):
  ...
  def __init__(self, embedding_size, activation_function='relu', add_extrinsics=False):
    super().__init__()
    self.add_extrinsics = add_extrinsics
    self.act_fn = getattr(F, activation_function)
    self.embedding_size = embedding_size
    ...
    if not self.add_extrinsics:
      self.fc = nn.Identity() if embedding_size == 1024 else nn.Linear(1024, embedding_size)
    else:
      self.fc = nn.Linear(1024+16, embedding_size)

  @jit.script_method
  def forward(self, observation, extrinsics = None):
...
def Encoder(symbolic, observation_size, embedding_size, activation_function='relu', add_extrinsics = False):
  # if symbolic:
  #   return SymbolicEncoder(observation_size, embedding_size, activation_function)
  # else:
  return VisualEncoder(embedding_size, activation_function, add_extrinsics)
